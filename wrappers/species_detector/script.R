...
cmd = paste0('( time blastn -max_hsps 1 -num_threads ',threads,' -max_target_seqs 1 -db ',ntdb,' -query ',fasta,' -out ',blast,' -outfmt "6 std staxids sscinames scomnames sblastnames" ) >> ',logfile,' 2>&1')
cat("## COMMAND: ",cmd,"\n")
shell(cmd)

# cmd = paste0('( sort -k 1,1 -k 12,12nr $FASTQ_BASE.blast > $FASTQ_BASE.blast.sorted && mv $FASTQ_BASE.blast.sorted $FASTQ_BASE.blast ) 2>> ')
colnames = c("query","target","pident","length","mismatch","gapopen","qstart","qend","tstart","tend","eval","bitscore","ttaxid","Species","tcomname","tblastname")
